...
func (r *HostedClusterReconciler) reconcile(ctx context.Context, req ctrl.Request, log logr.Logger, hcluster *hyperv1.HostedCluster) (ctrl.Result, error) {
	...
	{
		if len(hcluster.Spec.KasDNSName) > 0 {
...
func reconcileHostedControlPlane(hcp *hyperv1.HostedControlPlane, hcluster *hyperv1.HostedCluster, isAutoscalingNeeded bool) error {
	...
	// If ROSA passes KubeAPICustomKubeconfig.CustomKubeConfig, we should pass it through to the HCP, otherwise
	// we will keep it in blank for name auto-generation.
	// If ROSA passes the KasDNSName but KasDNSName it's not set in the HC, we will not set anything.
	if len(hcluster.Spec.KasDNSName) >= 0 {
		if hcp.Spec.KasCustomKubeconfig != nil && hcp.Spec.KasCustomKubeconfig.SecretRef != nil {
			hcp.Spec.KasCustomKubeconfig.KasDNSName = hcluster.Spec.KasDNSName
		} else {
			hcp.Spec.KasCustomKubeconfig = &hyperv1.CustomKubeconfig{
				KasDNSName: hcluster.Spec.KasDNSName,
			}
		}
	}
