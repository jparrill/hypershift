...
type KubeAPIServerParams struct {
	...
	// CustomizedExternalAddress is the external address that is customized by the user using the field KasDNSName.
...
func NewKubeAPIServerParams(ctx context.Context, hcp *hyperv1.HostedControlPlane, releaseImageProvider imageprovider.ReleaseImageProvider, externalAPIAddress string, externalAPIPort int32, externalOAuthAddress string, externalOAuthPort int32, setDefaultSecurityContext bool) *KubeAPIServerParams {
	...
	if hcp.Spec.KasCustomKubeconfig != nil && len(hcp.Spec.KasCustomKubeconfig.KasDNSName) > 0 {
		params.CustomizedExternalAddress = hcp.Spec.KasCustomKubeconfig.KasDNSName
	}
	...
	if hcp.Spec.KasCustomKubeconfig != nil {
		params.CustomKubeConfigRef = hcp.Spec.KasCustomKubeconfig.SecretRef
	}
