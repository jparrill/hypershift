...
func (r *reconciler) reconcileGlobalPullSecret(ctx context.Context, hcp *hyperv1.HostedControlPlane, cpoImage string) error {
	...
	if additionalPullSecretBytes, err = globalpullsecret.ValidateAdditionalPullSecret(additionalPullSecret); err != nil {
	...
	if globalPullSecretBytes, err = globalpullsecret.MergePullSecrets(originalPullSecretBytes, additionalPullSecretBytes); err != nil {
		return fmt.Errorf("failed to merge pull secrets: %w", err)
	}

	// Create secret in the DataPlane
	secret := manifests.GlobalPullSecret(UserProvidedPullSecretNamespace)
