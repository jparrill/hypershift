...
func TestStreamForName(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			streamName:     "rhel-8",
			expectError:    true,
			expectContains: "rhel-8",
		},
		{
			name: "When OSStreams is nil and name is non-empty it should return an error",
			releaseImage: &ReleaseImage{
				ImageStream: &imageapi.ImageStream{},
				StreamMetadata: &stream.Stream{
					Stream:        "rhcos-4.10",
		...
		{
			name: "When OSStreams entry has nil value it should return an error",
			releaseImage: &ReleaseImage{
				ImageStream: &imageapi.ImageStream{},
				StreamMetadata: &stream.Stream{
					Stream: "rhcos-4.10",
