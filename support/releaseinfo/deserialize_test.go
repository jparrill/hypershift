...
func TestDeserializeImageMetadata(t *testing.T) {
	tests := []struct {
		...
		{
			name: "When ConfigMap has only streams key it should populate OSStreams",
...
func TestDeserializeImageMetadataStreamsOnlyFallback(t *testing.T) {
	g := NewWithT(t)
	...
	// Simulate the fallback that registryclient_provider.go does:
	// when defaultStream is nil, pick the first non-nil stream from osStreams.
	if defaultStream == nil && len(osStreams) > 0 {
	...
	t.Run("When ConfigMap has only streams key StreamMetadata should be populated via fallback", func(t *testing.T) {
		g := NewWithT(t)
		g.Expect(ri.StreamMetadata).ToNot(BeNil())
		g.Expect(ri.StreamMetadata.Architectures).To(HaveKey("x86_64"))
	})

	t.Run("When ConfigMap has only streams key platform consumers should not panic", func(t *testing.T) {
		g := NewWithT(t)
		ami := ri.StreamMetadata.Architectures["x86_64"].Images.Aws.Regions["us-east-1"].Image
